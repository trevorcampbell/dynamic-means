	...
	public:
		MovingShapeDataGenerator(dmeans::Config cfg) : MovingDataGenerator(cfg){
			...
			for (uint32_t i = 0; i < this->alive.size(); i++){
					...
					shapes.push_back(static_cast<Shape>(4.0*uniformDist01(dmeans::RNG::get())));
			}
		}
			...
			switch(shapes[j]){
				case SQUARE:
					for (int k = 0; k < n; k++){
						...
						V2d newData = V2d::Zero();
						if (ang2 < 45.0*M_PI/180.0){
							newData(0) += radius;
							newData(1) += radius*tan(ang2);
						} else if (ang2 < 135.0*M_PI/180.0){
							ang2 -= M_PI/2.0;
							newData(0) -= radius*tan(ang2);
							newData(1) += radius;
						...
						//rotate the data
						V2d r;
						r(0) = newData(0)*cos(rots[j])-newData(1)*sin(rots[j]);
						r(1) = newData(0)*sin(rots[j])+newData(1)*cos(rots[j]);
						out.push_back(r+centers[j] + noise);
					}
					break;
				case LINE:
					for (int k = 0; k < n; k++){
				...
				case CIRCLE:
					for (int k = 0; k < n; k++){
						...
						V2d newData = V2d::Zero();
						double ang2 = uniformDistAng(dmeans::RNG::get());
						newData(0) += radius*cos(ang2);
						newData(1) += radius*sin(ang2);
						out.push_back(newData+centers[j]+noise);
					}
					break;
				default:
